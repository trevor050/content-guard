...
class ContentGuard {
  ...
  async extractNLPFeatures(content) {
    ...
    try {
      ...
      features.organizations = doc.organizations && doc.organizations().out('array') || []
      
      // Grammatical analysis - use safer methods
      features.questions = doc.questions().length
      features.exclamations = (content.allText.match(/!/g) || []).length // Manual count instead of doc.exclamations()
      features.contractions = doc.contractions ? doc.contractions().length : 0
