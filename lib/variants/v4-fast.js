...
class ContentGuardV4Fast {
  constructor(options = {}) {
    ...
    // Optimized obscenity detector
    this.obscenityFilter = obscenity;
  ...
  extractText(input) {
    ...
    return input.message || input.text || input.content || '';
  }
  ...
  enhancedObscenityCheck(text) {
    try {
      if (this.obscenityFilter.hasProhibited(text)) {
        return 4; // High confidence obscenity
      }
      return 0;
    } catch (error) {
      // Fallback to simple obscenity check
