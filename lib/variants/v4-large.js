...
class ContentGuardV4Large {
  ...
  createResult(score, riskLevel, processingTime, additionalData = {}, metadata = {}) {
    ...
    return {
      ...
      preset: {
        ...this.preset,
        spamThreshold: this.options.spamThreshold // CRITICAL: Include the threshold to fix N/A display
      },
      metadata: {
        ...metadata,
  ...
  /**
   * Register lightweight inline plugins - CRITICAL MISSING PIECE for v4-large
   */
  ...
  // NEW: Enhanced L33tspeak Analysis - Test all variations (Same as v4-balanced)
  ...
  // Score a single l33tspeak variation for toxic content (Same as v4-balanced)
  scoreLeetSpeakVariation(variation, content) {
    ...
    combinedPatterns.forEach(({ pattern, score: patternScore, name, key }) => {
      if (pattern.test(variation) && !detectedTerms.has(key)) {
        detectedTerms.add(key)
        score += patternScore
        flags.push(`L33T-COMBO: ${name} decoded from l33tspeak`)
      }
    })

    return { score, flags, detectedTerms: Array.from(detectedTerms) }
  }
}
