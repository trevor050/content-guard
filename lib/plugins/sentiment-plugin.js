...
class SentimentPlugin {
  constructor() {
    ...
    this.options = {}
    this.sentimentAnalyzer = null // Initialize as null
  }
  ...
  async init(config = {}) {
    ...
    this.options = {
      ...
      moderateThreshold: config.moderateThreshold || -0.1,
    ...
    try {
      ...
      this.sentimentAnalyzer = this.createFallbackSentiment()
      this.initialized = true
    }
  }
  ...
  async analyze(content, input, globalOptions = {}) {
    ...
    try {
      // Ensure we have a working sentiment analyzer
      if (!this.sentimentAnalyzer || typeof this.sentimentAnalyzer !== 'function') {
        this.sentimentAnalyzer = this.createFallbackSentiment()
      }
  ...
  createFallbackSentiment() {
    ...
    return function(text) {
      if (!text || typeof text !== 'string') {
        return { normalizedScore: 0, words: [] }
      }
