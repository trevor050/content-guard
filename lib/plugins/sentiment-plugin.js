...
class SentimentPlugin {
  ...
  async init(options = {}) {
    this.options = {
      negativeThreshold: options.negativeThreshold || -0.5,
      moderateThreshold: options.moderateThreshold || -0.2,
      contextAware: options.contextAware !== false,
      weight: options.weight || 1.0,
      ...options
    }

    // Lazy load wink-sentiment
    if (!winkSentiment) {
      try {
        winkSentiment = require('wink-sentiment')
      } catch (error) {
        console.warn('wink-sentiment not available, falling back to simple sentiment')
        winkSentiment = this.createFallbackSentiment()
      }
    }
  ...
  async analyze(content, input, globalOptions = {}) {
    ...
    try {
      ...
      const result = winkSentiment(content.allText)
