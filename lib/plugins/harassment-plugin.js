...
const CONDESCENDING_PATTERNS = [
  ...
  // MISSING FROM BENCHMARK: Key workplace condescension patterns
  /maybe someone else should take the lead on this since it's pretty technical/i,
  /you probably don't understand the nuances of this industry like the rest of us do/i,
  /are you sure you can handle this level of responsibility given your experience/i,
  /this might be too much for you - do you want me to assign it to someone else/i,
  /you look tired - is everything okay at home affecting your work/i,
  /given your experience level, i think we should have someone else review your work/i,
  /you might want to consider whether you're really cut out for this type of role/i,
  /you seem stressed lately - are you having trouble keeping up with the workload/i,
  /maybe someone else should take the lead.*since it's.*technical/i,
  /you probably don't understand.*nuances.*industry/i,
  /handle this level of responsibility.*given your experience/i,
  /might be too much for you.*assign.*someone else/i,
  /look tired.*everything okay at home.*affecting.*work/i,
  /experience level.*someone else review.*work/i,
  /consider whether you're really cut out.*type of role/i,
  /seem stressed.*trouble keeping up.*workload/i,
  ...
  // BENCHMARK MISSING: Coded identity harassment patterns
  /you bring such a unique perspective to our traditional workplace environment/i,
  /it's great to see diversity working out so well in our technical department/i,
  /you're so well-spoken and articulate - much better than i expected initially/i,
  /i'm impressed by how professional you've become since starting here/i,
  /you fit in surprisingly well with our established company culture and values/i,
  /your work ethic is quite impressive for someone with your background/i,
  /you've really adapted well to our professional standards and expectations/i,
  /it's refreshing to work with someone who defies the usual stereotypes/i,
  /bring such a.*unique perspective.*traditional workplace/i,
  /diversity working out so well.*technical department/i,
  /well-spoken.*articulate.*better than.*expected/i,
  /impressed.*professional.*become since starting/i,
  /fit in surprisingly well.*company culture/i,
  /work ethic.*impressive.*someone with your background/i,
  /adapted well.*professional standards/i,
  /refreshing.*defies.*usual stereotypes/i
]
...
const GASLIGHTING_PATTERNS = [
  // Denying reality - ENHANCED FROM BENCHMARK ANALYSIS
  ...
  // MISSING CRITICAL BENCHMARK PATTERNS: Direct gaslighting
  /you're clearly overreacting to what was obviously just constructive feedback/i,
  /i never said that - you must be misremembering our conversation/i,
  /everyone else on the team understands this perfectly fine, so the problem must be with you/i,
  /you're being way too sensitive about this - no one else has any issues/i,
  /you're projecting your own insecurities onto this situation/i,
  /maybe you should take a step back and consider whether you're being rational/i,
  /your emotional response is making it difficult to have a productive conversation/i,
  /that's not what happened and you know it - you're distorting the facts/i,
  /clearly overreacting.*obviously.*constructive feedback/i,
  /never said that.*misremembering our conversation/i,
  /everyone else.*understands.*perfectly.*problem.*with you/i,
  /being way too sensitive.*no one else.*issues/i,
  /projecting.*insecurities.*situation/i,
  /step back.*consider.*being rational/i,
  /emotional response.*difficult.*productive conversation/i,
  /not what happened.*know it.*distorting.*facts/i,
...
const VEILED_THREAT_PATTERNS = [
  // Consequences and implications - ENHANCED FROM BENCHMARK ANALYSIS
  ...
  // MISSING CRITICAL BENCHMARK PATTERNS: Veiled threats
  /it would be unfortunate if certain information about your performance became widely known/i,
  /i wonder what your colleagues would think if they knew about your situation at home/i,
  /accidents happen all the time to people who don't know when to keep quiet/i,
  /you seem like the type who wouldn't want their personal information floating around/i,
  /it's interesting how easy it is to find someone's address and family details these days/i,
  /i hope nothing bad happens to disrupt your comfortable little life and career/i,
  /people who cause problems for others sometimes find that problems come back to them/i,
  /it would be a shame if your reputation in this industry were to be permanently damaged/i,
  /unfortunate.*certain information.*performance.*widely known/i,
  /wonder what.*colleagues.*think.*knew.*situation at home/i,
  /accidents happen.*people.*don't know.*keep quiet/i,
  /type who wouldn't want.*personal information.*floating around/i,
  /easy.*find.*address.*family details/i,
  /hope nothing bad happens.*disrupt.*comfortable.*life.*career/i,
  /cause problems.*others.*problems come back/i,
  /shame.*reputation.*industry.*permanently damaged/i,
...
const POWER_DYNAMIC_PATTERNS = [
  ...
  // MISSING CRITICAL BENCHMARK PATTERNS: Core power abuse
  /remember who signs your paycheck before you decide to push back on my decisions/i,
  /you work for me, so i suggest you adjust your attitude and follow my instructions/i,
  /i can make your life very difficult here if you continue to question my authority/i,
  /you're easily replaceable, and there are plenty of people who would love this job/i,
  /good luck finding another job in this industry after i'm done with you/i,
  /i own this department and everyone in it, including you/i,
  /you serve at my pleasure, and that pleasure is rapidly diminishing/i,
  /don't forget who's in charge here and what that means for your future/i,
  /remember who signs your paycheck.*push back.*decisions/i,
  /work for me.*adjust your attitude.*follow.*instructions/i,
  /make your life.*difficult.*continue.*question.*authority/i,
  /easily replaceable.*plenty of people.*love this job/i,
  /good luck finding.*job.*industry.*done with you/i,
  /own this department.*everyone.*including you/i,
  /serve at my pleasure.*rapidly diminishing/i,
  /don't forget.*in charge.*means.*future/i,
...
const SOCIAL_EXCLUSION_PATTERNS = [
  ...
  // MISSING CRITICAL BENCHMARK PATTERNS: Team exclusion
  /nobody on the team really wants to work with you on important projects/i,
  /everyone's been talking about your attitude problems behind your back/i,
  /you don't really fit in with our group dynamic here/i,
  /the team doesn't trust you with sensitive or important tasks/i,
  /you're not really one of us, and it shows in how you approach work/i,
  /colleagues have been complaining about having to collaborate with you/i,
  /people are starting to notice your behavior and it's affecting team morale/i,
  /you're making everyone uncomfortable with your approach to things/i,
  /nobody.*team.*wants to work.*important projects/i,
  /everyone.*talking.*attitude problems.*behind your back/i,
  /don't really fit.*group dynamic/i,
  /team doesn't trust.*sensitive.*important tasks/i,
  /not really one of us.*shows.*approach work/i,
  /colleagues.*complaining.*collaborate with you/i,
  /people.*starting to notice.*behavior.*team morale/i,
  /making everyone uncomfortable.*approach/i,
