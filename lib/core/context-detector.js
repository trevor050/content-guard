...
class ContextDetector {
  ...
  detectDomains(content) {
    ...
    for (const [domain, vocabulary] of Object.entries(EXTENDED_CONTEXTS)) {
      ...
      const threshold = Math.max(1, vocabulary.length * 0.1) // At least 10% match or 1 term
    ...
    // Enhanced: Also check against global technical terms for immediate domain detection
    const { TECHNICAL_TERMS } = require('../constants/context-data')
    ...
    // Enhanced: Also check against global business terms for immediate domain detection
    const { BUSINESS_TERMS } = require('../constants/context-data')
